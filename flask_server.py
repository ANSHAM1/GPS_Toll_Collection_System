...
def receive_gps():
    ...
    if request.is_json:
        ...
        if data and 'latitude' in data and 'longitude' in data:
            ...
            latest_coordinates = {"latitude": data['longitude'], "longitude": data['latitude'], 'distance': org.entity.distance_travelled}

            print(f"Received GPS coordinates: Latitude={latitude}, Longitude={longitude}")
            car_travelling_on_toll_road([latitude,longitude])

            return jsonify({"message": "coordinate received"}), 200
        else:
...
@app.route('/car_path')
def car_path():
    return render_template('car_path_map.html')

@app.route('/square_zones')
def zones():
    return render_template('square_zone_map.html')

@app.route('/toll_roads')
def toll_roads():
    return render_template('toll_roads_map.html')
